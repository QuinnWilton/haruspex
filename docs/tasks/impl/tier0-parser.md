...
- **Top-level**: `def` (with optional `@total`, `@extern`, `@private`), `type` declarations, `mutual do ... end` blocks, `import` statements, `variable` declarations, `class`/`instance` declarations, `record` declarations
- **Expressions**: variables, literals, function application `f(x, y)`, lambdas `fn(x) do ... end`, let bindings, case/if/else, binary operators, unary operators, pipe `|>`, type annotations `(e : T)`, holes `_`, with-expressions
...
2. **Error recovery**: on parse error, skip tokens until `end`, `def`, `type`, or EOF. Collect up to 10 errors before giving up. Each error includes the span of the unexpected token.
...
- **Pipe**: `x |> f |> g` → `g(f(x))`
- **Lambda**: `fn(x, y) do x + y end` → correct AST
...
- **Type declarations**: `type Option(a) do none; some(a) end`
...
- **Variable declarations**: `variable {a : Type}`
...
- Multiple errors collected up to limit
